...
void setup() {
  ...
  state.mainsPowerPresent = readMainsPowerPresent();
...
void loop() {
  ...
  if (nowMs - state.lastMainsCheckMs >= MAINS_CHECK_INTERVAL_MS) {
    state.lastMainsCheckMs = nowMs;
    handlePowerFailMonitoring();
  }

  if (nowMs - state.lastKeypadPollMs >= KEYPAD_POLL_INTERVAL_MS) {
  ...
  StaticJsonDocument<3072> cfgDoc;
